...
// BFSTail->next == NULL;
 ...
 //BFSTreeCollect(tid, v, *BFSHead, *BFSTail); // invoke the BFS tree collect
  ...
  int status = Scan(tid, v, *BFSHead, *BFSTail); 
  napp++;
  if(status == 2){
 //PrintBFSTree(tid, BFSHead, BFSTail); // Print the BFS-tree
 //napp++;
...
void locateE(vlist_t *startV, elist_t ** n1, elist_t ** n2, int key){
       elist_t  *currv, *predv;
        predv = startV->enext.load();
        currv = predv->enext.load();
        while(currv && currv->key < key){
         predv = currv;
         currv = (elist_t*)get_unmarked_ref((long)currv->enext.load());
    }
    //if(currv->key >= key){
          (*n1) = predv;
          (*n2) = currv;
...
// add a new edge in the edge-list  for init graph  
 char * PutEInit(int key1, int key2){
               elist_t* prede, *curre;
               vlist_t *u,*v;
...
/*
// init of graph from file
 void initGraphFromFile(string file, int NT){
  ifstream cinn(file);
  long n,m;
  int u, v;
 cinn>>n>>m;
 cout<<n<<" "<<m<<endl;
  ...
  for( j=1; j<=m; j = j+1){
   ...
   // add a new vertex in the vertex-list
 char* PutVEInitGraph(string file, int NT){
	ifstream cinn(file);
  	long n,m;
  	int u, v;
 	cinn>>n>>m;
	...
	currv = predv->vnext;  
   	...
   	//for( i=1;i<=n;i++){
        ...
        //} 
	while(i != n){
           vlist_t *newv = createV(i, NT); // create a new vertex node
           newv->vnext.store(currv);  
	   predv->vnext = newv;
	   predv = predv->vnext;
	   currv = predv->vnext;
...
// Init for each vertex   
void Init(int tid){
        vlist_t *currv = Head->vnext;
          while(currv->vnext.load() != NULL){
           if(!is_marked_ref((long) currv->vnext.load())){
             //currv->starttime[tid] = 0;
             //currv->endtime[tid] = 0;
             currv->visitedArray[tid] = INT_MAX;
             }
           currv =  (vlist_t *) get_unmarked_ref((long)currv->vnext.load());
 ...
 void BFSTreeCollect(int tid, vlist_t *u, bfslist_t &bfsHead, bfslist_t &bfsTail){
    elist_t * eHead;
    bfslist_t *bfsHead1 = &bfsHead, *bfsTail1 = &bfsTail;
    //int i;
   // int time = 1;
   ...
   // u->starttime[tid] = time;
    Q.push(u);
    while(!Q.empty()){ // run until que is not empty
     VNode * currentVNode = Q.front(); // get the front
     Q.pop(); // deque
      if(checkVisited(tid, currentVNode, cnt)) // check visited or n
        continue;
      currentVNode->visitedArray[tid] = cnt; // mark visited
      bfslist_t *bfsNode = createBFSNode(currentVNode,NULL,NULL);
      ADDToBFSTree(&bfsHead1, &bfsTail1, bfsNode);// add to the BFS-tree
      
      eHead = currentVNode->enext.load(); // Ehead
      for(elist_t * itNode = eHead->enext.load(); itNode->enext.load() != NULL; itNode = (elist_t*)get_unmarked_ref((long)itNode ->enext.load())){ // iterate through all adjacency vertices 
      //  cout<<"itNode:"<<itNode->key<<endl;
        if(!is_marked_ref((long) itNode->enext.load())){  
          vlist_t *adjVNode = itNode->pointv.load(); // get the vertex
	   ...
	   if(!checkVisited(tid,adjVNode, cnt) ){ // check vertex is visited or not
                   ...
                   //time = time + 1;  // inc time counter
                   Q.push(adjVNode); // enque to Q
                  }
                       
                }
               //currentVNode ->endtime[tid] = time + 1;
               //time = time + 1;  
              }   
           }
          } 
...
//int i;
 bfslist_t * ListH1 = ListH->next;
  while(ListH1->next != NULL){
    coutt<<ListH1->n->key;//<<","<<ListH1->n->starttime[tid]<<","<<ListH1->n->endtime[tid]<<"->";
    ListH1 = ListH1->next;
  }
  coutt<<endl;
}      
